            ...
            for d in range(n_days):
                for h1 in range(n_hours):
                        for j3 in range(n_players):
                            for h2 in range(n_hours):
                                if h1 != h2 and j1 != j2 and j1 != j3 and j2 != j3:
                                    print("f")
    return rest_3
            ...
            for j3 in range(n_players):
                for d in range(n_days-1):
                    for h1 in range(n_hours):
                        for h2 in range(n_hours):
                            if j1 != j2 and j1 != j3:
                                x = possible_games.index((j1, j2, d, h1)) + 1
                                y = possible_games.index((j1, j3, d+1, h2)) + 1
                                
                                rest_4.append(f"-{x} - {y} 0")

                                x = possible_games.index((j2, j1, d, h1)) + 1
                                y = possible_games.index((j3, j1, d+1, h2)) + 1
